...
import { ArrowLeft, Plus, Edit2, Trash2, User, Mail, Download, Send, Clock, TriangleAlert } from 'lucide-react';
...
const VotersPage = () => {
  ...
  const [deleteDialog, setDeleteDialog] = useState({ isOpen: false, voterId: null, voterName: '' });
  ...
  const handleDelete = async () => {
    if (!deleteDialog.voterId) return;

    try {
      await usersAPI.delete(deleteDialog.voterId);
      ...
      setDeleteDialog({ isOpen: false, voterId: null, voterName: '' });
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <AlertDialog 
        ...
        onOpenChange={(open) => setDeleteDialog({ isOpen: open, voterId: null, voterName: '' })}
      >
        <AlertDialogContent>
          <AlertDialogHeader className="items-center">
            ...
            <AlertDialogTitle>Supprimer cet électeur ?</AlertDialogTitle>
            <AlertDialogDescription className="text-center">
              Cette action ne peut pas être annulée. Cela supprimera définitivement l'électeur
              <strong> {deleteDialog.voterName}</strong> et toutes ses données associées.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
